...
class AddTimeViewModel : ViewModel() {
    ...
    private val temporaryStorage = MutableLiveData<ArrayList<TemporaryStorageTime>>()
...
//            if (data.isEmpty()) {
//                value = value?.apply {
//                    add(TemporaryStorageTime())
//                    value?.get(0)?.number = 0
//                }
//            } else {
//                val list = arrayListOf<TemporaryStorageTime>()
//                for ((i, v) in data.withIndex()) {
//                    list.add(TemporaryStorageTime())
...
//        }
    ...
    fun deleteLine(size: Int) {
        with(temporaryStorage) {
            if (size != 0) {
                value = value?.apply { value?.removeAt(size) }
            }
        }
    }
